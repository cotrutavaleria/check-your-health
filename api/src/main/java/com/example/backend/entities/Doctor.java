...
public class Doctor implements Serializable {
    ...
    @ManyToMany(targetEntity = DoctorTimetable.class)
    Set<DoctorTimetable> schedules;
    @OneToMany(targetEntity = Appointment.class)
    Set<Appointment> appointments;
    @OneToMany(targetEntity = Review.class)
    Set<Review> reviews;
    @OneToMany(targetEntity = ConsultingType.class)
    ...
    private Integer id;
    @NotBlank(message = "Status must be DOCTOR.")
    private UserType status;
    @NotBlank(message = "Specialty should not be null.")
    @NotNull
    private String specialty;
    @NotBlank(message = "Work address should not be null.")
    @NotNull
